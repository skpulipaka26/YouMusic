...
export const addSongToPlaylist = song => {
  return async (dispatch, getState) => {
    ...
    if (
      ...
      song.videoId !== extractorData.metadata.videoId
    ) {
    ...
    const currSongIndex = currPlaylist.findIndex(
...
export const setupCurrentPlayingPlayer = extractorData => {
  return dispatch => {
    ...
    const howl = new Howl({
      ...
      format: formats.map(
        format => format.mimeType.split(';')[0].split('/')[1]
      ),
      html5: true
    });
    const events = ['play', 'pause', 'stop', 'end', 'loaderror', 'playerror'];
    events.forEach(event =>
      dispatch(setupPlayerEvents(event, metadata.videoId, howl))
    );
    return howl;
  };
};
